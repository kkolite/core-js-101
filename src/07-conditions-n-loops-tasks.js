...
function doRectanglesOverlap(rect1, rect2) {
  ...
  for (let i = rect1.top; i <= rect1.top + rect1.height; i += 1) {
    for (let k = rect1.left; k <= rect1.left + rect1.width; k += 1) {
      firstCoord.push([k, i]);
    }
  }
  for (let i = rect2.top; i <= rect2.top + rect2.height; i += 1) {
    for (let k = rect2.left; k <= rect2.left + rect2.width; k += 1) {
      secondCoord.push([k, i]);
    }
  }
  secondCoord.forEach((el) => {
    for (let i = 0; i < firstCoord.length; i += 1) {
      if (firstCoord[i][0] === el[0] && firstCoord[i][1] === el[1]) {
        result = true;
        break;
      }
    }
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (brackets[str[i]] === top && stack.length >= 1) {
      stack.pop();
